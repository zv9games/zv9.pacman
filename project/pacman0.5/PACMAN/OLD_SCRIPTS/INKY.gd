...
@onready var anisprite = $"/root/BINARY/ORIGINAL/CHARACTERS/INKY/AnimatedSprite2D"
...
func initialize():
	if is_initialized:
		return
	for key in inky_data.keys():
		for i in range(inky_data[key].size()):
			inky_data[key][i] = tile_position_to_global_position(inky_data[key][i])
