...
var start_position = Vector2(17, 18)
var pinky_position = start_position * tile_size
var is_frozen = false
var ghost_state = FrightStates.NORMAL
var dot_eaten_count = 0     
var current_state = null             
var scatter_index = 0
var pinky_data: Dictionary = { "home_corner_loop": [Vector2(9, 8), Vector2(9, 3), Vector2(3, 3), Vector2(3, 8)] }
var score: int = 0
var last_state  # Variable to store the last state
var is_eaten = false  
var pinky_initial_target: Vector2
var pinky_initial_positions: Array = [Vector2(17, 18), Vector2(17, 15)]
var pinky_initial_index: int = 0
var is_initialized = false

@onready var zpu = $"/root/BINARY/GAME/ZPU"
@onready var gamestate = $"/root/BINARY/GAME/GAMESTATE"
@onready var nav_agent = $/root/BINARY/LEVELS/ORIGINAL/CHARACTERS/PINKY/NavigationAgent2D 
...
@onready var animatedsprite = $/root/BINARY/LEVELS/ORIGINAL/CHARACTERS/PINKY/AnimatedSprite2D  
@onready var scoremachine = $/root/BINARY/GAME/SCOREMACHINE
@onready var collision_area = $/root/BINARY/LEVELS/ORIGINAL/CHARACTERS/PINKY/Area2D  
...
func _start_initial_behavior():
	self.visible = true
	self.position = pinky_position
	pinky_initial_target = tile_position_to_global_position(pinky_initial_positions[pinky_initial_index])
	move_blinky_initial()
...
func move_blinky_initial() -> void:
	var dir = (pinky_initial_target - global_position).normalized()
	velocity = dir * speed
	move_and_slide()
	if global_position.distance_to(pinky_initial_target) < 1:
		pinky_initial_index = (pinky_initial_index + 1) % pinky_initial_positions.size()
		pinky_initial_target = tile_position_to_global_position(pinky_initial_positions[pinky_initial_index])
...
func make_scatter_path() -> void:
	scatter_index = (scatter_index + 1) % pinky_data["home_corner_loop"].size()
	var target_pos = pinky_data["home_corner_loop"][scatter_index]
...
func move_away_from_pacman() -> void:
	...
	move_and_slide()
	animatedsprite.play("frightened")  # Ensure frightened animation is playing
...
func move_to_shed() -> void:
	...
	if next_position != Vector2.ZERO:
		...
		move_and_slide()
		if dir.x > 0:
			animatedsprite.play("eaten_right")
		else:
			animatedsprite.play("eaten_left")
		if global_position.distance_to(nav_agent.target_position) < 1:
			reset_to_normal_state()
			_start_initial_behavior()  # Call the initial behavior function here
	else:
		pass
...
func set_freeze(freeze: bool) -> void:
	is_frozen = freeze
	if is_frozen:
		animatedsprite.stop()  # Stop animation when frozen
