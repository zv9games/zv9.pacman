...
@onready var anisprite = $"/root/BINARY/ORIGINAL/CHARACTERS/PINKY/AnimatedSprite2D"  
...
func initialize():
	if is_initialized:
		return
	for key in pinky_data.keys():
		for i in range(pinky_data[key].size()):
			pinky_data[key][i] = tile_position_to_global_position(pinky_data[key][i])
